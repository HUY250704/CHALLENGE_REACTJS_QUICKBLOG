...
export default function Home() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 pb-10 pt-8 sm:px-6 lg:pt-10">
      <div className="mx-auto max-w-3xl text-center">
        <h1 className="text-4xl font-bold leading-[1.08] tracking-normal text-slate-700 dark:text-white sm:text-6xl">
          Your own <span className="text-indigo-600">blogging</span> platform.
        </h1>
        <p className="mx-auto mt-5 max-w-3xl text-base leading-7 text-slate-500 dark:text-slate-300">
        ...
        <form
          className="mx-auto mt-7 flex max-w-2xl overflow-hidden rounded-md border border-slate-300 bg-white p-1 shadow-sm dark:border-slate-700 dark:bg-slate-950"
          onSubmit={(event) => {
        ...
        >
          <Input
            className="h-12 min-w-0 border-0 shadow-none focus:border-0 focus:ring-0"
            placeholder="Enter search title..."
            value={query}
            onChange={(event) => setQuery(event.target.value)}
          />
          <Button className="h-12 min-w-[8rem] shrink-0 rounded-md text-base">
            Search
          </Button>
        </form>
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {filteredPosts.map((post) => (
            <CardBlog key={post._id || post.id} post={post} />
          ))}
        </div>
      ) : (
